...
const elements = {
    ...
    otherStates2: document.getElementById("other-states2")
};
...
function createBarChart2(data, container, options = {}) {
    const {
        xValue = d => d.name,
        yValue = d => d.value,
        xLabel = '',
    ...
    const margin = MAP_CONFIG.chart2.margin;
    const width = MAP_CONFIG.chart2.width - margin.left - margin.right;
    const height = MAP_CONFIG.chart2.height - margin.top - margin.bottom;

    // Create SVG
    const svg = d3.select(container)
        .append('svg')
        .attr('width', MAP_CONFIG.chart2.width)
        .attr('height', MAP_CONFIG.chart2.height)
        .append('g')
        .attr('transform', `translate(${margin.left},${margin.top})`);
    ...
    svg.selectAll('.bar')
        ...
        .attr('width', x.bandwidth())
        .attr('height', d => height - y(yValue(d)))
        .attr('fill', d => highlightBar(d) ? '#2ca25f' : MAP_CONFIG.chart2.barColor) // Highlight selected bird
        .attr('stroke', d => highlightBar(d) ? '#006d2c' : 'none') // Add border to selected bird
        .attr('stroke-width', d => highlightBar(d) ? 2 : 0)
        // .on('mouseover', function(event, d) {
        //     d3.select(this)
        //         .transition()
        //         .duration(200)
        //         .attr('fill', highlightBar(d) ? '#006d2c' : MAP_CONFIG.chart2.barHoverColor);
        ...
        .on('mouseout', function(event, d) {
            d3.select(this)
                .transition()
                .duration(200)
                .attr('fill', highlightBar(d) ? '#2ca25f' : MAP_CONFIG.chart2.barColor);
...
function getBirdInfo(birdNames, data) {
    let filteredBirds = [];
    
    for (let i = 0; i < birdNames.length; i++) {
        for (let j = 0; j < data.length; j++) {
            if (data[j].BirdName === birdNames[i]) {
                filteredBirds.push(data[j]);
            }
        }
    }
...
function updateBirdInfo(bird, data) {
    try {
        ...
        elements.otherStates.innerHTML = `
        <h4>Distribution by State</h4>
        <div id="bird-chart2" class="chart-container"></div>
    `;

    // Prepare data for the chart
    const chartData2 = stateData
        .filter(d => parseInt(d.NumBird) > 0)
        .slice(0, 10) // Show top 10 states
        .map(d => ({
            name: d.State,
    ...
    // Create the chart
    createBarChart2(chartData2, '#bird-chart2', {
        xLabel: 'States',
        yLabel: 'Number of Birds',
        rotateLabels: true
...
function updateStateInfo(stateName, data ,bird_info) {
    // Get all birds in the state
    const birdsInState = getBirdsInState(stateName, data)
        .sort((a, b) => parseInt(b.NumBird) - parseInt(a.NumBird))
        .slice(0, 10); // Show top 10 birds
    
    const info = getBirdInfo(birdsInState.map(item=>item.BirdName),bird_info);
    // If a bird is currently selected, highlight it in the chart
    const selectedBird = "All States";
    console.log(birdsInState.map(item=>item.BirdName))
    console.log(info)
    elements.otherStates2.innerHTML = `
    <h4 margin-top="80px">Life span of birds in ${stateName}</h4>
    <div id="state-chart2" class="chart-container"></div>
`;
    const chartData2 = info.map(d => ({
        name: d.BirdName,
        value: parseInt(d.LifeSpan),
        isSelected: d.BirdName === selectedBird
    }));
    console.log(chartData2)
    // Create the chart with highlighting for selected bird
    createBarChart2(chartData2, '#state-chart2', {
        xLabel: 'Bird Species',
        yLabel: 'Lifespan',
        rotateLabels: true,
...
function drawMap(us,bird_info, allBirdData) {
    try {
        ...
        svg.selectAll("path")
            ...
            .attr("fill", d => {
                ...
                return count === 0 ? "#0072B2" : colorScale(count);
            })
            ...
            .on("click", (event, d) => {
                if (d?.properties?.NAME) {
                    ...
                    updateStateInfo(d.properties.NAME, allBirdData,bird_info);
                }
            });
...
async function initializeApp() {
    try {
        const [data,bird_info, us] = await Promise.all([
            d3.csv(PATHS.DATA),
            d3.csv(PATHS.BIRD_INFO),
            d3.json(PATHS.US_STATES)
        ]);

        //populateDropdown(data);
        drawMap(us,bird_info, data);
